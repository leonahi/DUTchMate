...
HARNESS_SOURCE = Path(__file__).with_name("control_state_harness.c")
...
def _build_harness(tmp_path: Path) -> Path:
    ...
    executable = tmp_path / "debug-helper-control-state"
...
def test_control_state_machine(tmp_path: Path, scenario: str) -> None:
