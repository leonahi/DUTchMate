...
HARNESS_SOURCE = Path(__file__).with_name("cdc_tx_state_harness.c")
...
def _build_harness(tmp_path: Path) -> Path:
    ...
    executable = tmp_path / "debug-helper-cdc-tx-state"
    result = subprocess.run(
        [
            ...
            str(FIRMWARE_SOURCE),
            str(HARNESS_SOURCE),
            str(FIRMWARE_SOURCE / "cdc_tx_state.c"),
            "-o",
            str(executable),
        ],
        check=False,
...
def test_cdc_tx_state_machine(tmp_path: Path, scenario: str) -> None:
