...
8. Add the CLI as a thin HTTP client. **In progress: start, stop, device discovery, status, capture, boot-test, GPIO mode, reset, and boot-mode commands are implemented; log/session, wait-pattern, and UART-send commands remain.**
