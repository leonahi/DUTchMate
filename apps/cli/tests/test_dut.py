...
def test_format_reset_result_renders_timestamp() -> None:
    assert (
        format_reset_result({"ok": True, "timestamp_us": 182334500})
        == "Reset command accepted (timestamp_us=182334500)"
    )


def test_format_boot_mode_result_renders_mode_and_timestamp() -> None:
    assert (
        format_boot_mode_result("bootloader", {"ok": True, "timestamp_us": 182334600})
        == "Boot mode set to bootloader (timestamp_us=182334600)"
    )
...
def test_dut_reset_command_pulses_reset_role(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_reset_dut(*, pulse_ms: int, service_url: str) -> dict[str, object]:
        ...
        return {"ok": True, "timestamp_us": 182334500}
...
def test_dut_boot_mode_command_sets_boot_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_set_boot_mode(*, mode: str, service_url: str) -> dict[str, object]:
        ...
        assert service_url == "http://127.0.0.1:2040"
        return {"ok": True, "timestamp_us": 182334600}

    monkeypatch.setattr(main, "set_boot_mode", fake_set_boot_mode)
    monkeypatch.setattr(
        main,
        "format_boot_mode_result",
        lambda mode, _payload: f"Boot mode set to {mode}",
    )
