...
## Corpus Check
- 390 files · ~298,550 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5411 nodes · 13933 edges · 271 communities (250 shown, 21 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 2430 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `10a8b847`
...
## Communities (271 total, 21 thin omitted)

### Community 0 - "test_backend_reconnect.py"
Cohesion: 0.09
Nodes (28): BackendReconnectCoordinator, Serialize idle and active reconnect attempts for one stable source owner., Start the single non-daemon idle reconnect worker., Own one bounded active-workflow reconnect attempt., Stop reconnect activity and join the idle worker exactly once., ClosableSource, FakeClock, FakeSourceOwner (+20 more)
...
Cohesion: 0.19
Nodes (18): CliConfig, load_cli_config(), parse_cli_config(), Merged CLI configuration., Load CLI configuration from TOML, returning defaults when missing., Parse a raw TOML mapping into validated CLI configuration., parametrize, Path (+10 more)
...
Cohesion: 0.11
Nodes (57): RuntimeError, Raised when the CLI cannot complete a Device Core Service request., Raised when the local Device Core Service cannot be reached., ServiceClientError, ServiceUnavailableError, _display(), format_boot_mode_result(), format_reset_result() (+49 more)

### Community 5 - "ContinuousIngestionCoordinator"
Cohesion: 0.05
Nodes (85): ContinuousIngestionCoordinator, BackendEvent, Return one immutable health snapshot for the installed source., Establish a fresh cursor for one finite workflow., Return to idle draining and discard unread workflow events., Return the next active-workflow event or an inactivity timeout., Retain active events because workflow activation owns cursor freshness., Wait until an idle source disconnect can be claimed for replacement. (+77 more)
...
Cohesion: 0.22
Nodes (19): BackendConfigError, _baudrate(), _boolean(), _finite_number(), _fixed_integer(), _fixed_string(), _optional_baudrate(), _optional_table() (+11 more)
...
Cohesion: 0.09
Nodes (59): DeviceCoreRuntime, Compose Phase 1 core services behind one service-facing object., Capture-session storage used by this runtime., Return one bounded page without requiring a backend connection., Return recent UART evidence without requiring a backend connection., Designate a stored session without requiring a backend connection., Clear a named designation without requiring a backend connection., Compare stored evidence without requiring a backend connection. (+51 more)

### Community 11 - "create_app"
Cohesion: 0.05
Nodes (93): _close_runtime(), create_app(), FastAPI, Path, FastAPI application factory for the Device Core Service., Create the Device Core Service application., DUTchMate Device Core Service package., baseline_mutation_payload() (+85 more)

### Community 12 - "enhanced.py"
Cohesion: 0.06
Nodes (44): BufferOverflowEvent, Observed Enhanced-backend UART receive-buffer loss., _control_success_timestamp(), normalize_enhanced_message(), BackendEvent, DeviceMessage, Enhanced v1 protocol adapters and normalized event translation., Parse a chunk and normalize its UART and buffer-telemetry messages. (+36 more)
...
### Community 14 - "store.py"
Cohesion: 0.03
Nodes (65): Return bounded recent UART replay for one selected session., Designate one eligible session as the project baseline., Clear the baseline only when it names the requested session., Compare one terminal session with the designated baseline., capture_summary_payload(), Serialize a completed capture session summary., Serialize one successful matched or unmatched wait-pattern outcome., wait_pattern_payload() (+57 more)

### Community 15 - "SegmentContext"
Cohesion: 0.06
Nodes (22): EnhancedReconnectHost, Ready async Enhanced host used as source, control, and UART sender., _close_source(), Exception, Transfer one validated replacement into the stable coordinator., Return the latest timestamp provenance published by the source., _source_segment(), Return timestamp provenance once the source origin is established. (+14 more)

### Community 16 - "validation.py"
Cohesion: 0.05
Nodes (67): BootMode, GpioIdentifierValidationError, _is_unicode_whitespace(), prepare_uart_send_payload(), GpioControlChannel, GpioRoleName, ValueError, Shared validation for public Device Core input contracts. (+59 more)
...
### Community 20 - "DeviceCoreRuntimeError"
Cohesion: 0.09
Nodes (16): DeviceCoreRuntimeError, BackendCapability, GpioModeRequestSource, GpioRoleName, RuntimeError, SessionWorkflow, Close the currently owned backend source exactly once., Apply configured GPIO modes after a Debug Helper hello is available. (+8 more)

### Community 21 - "log_replay.py"
Cohesion: 0.12
Nodes (45): SessionDetail, _decode_uart_event(), _existing_paths(), _latest_terminal_native(), _non_negative_int(), _normal_record(), _oversized_record(), Path (+37 more)
...
Cohesion: 0.07
Nodes (19): Protocol, SessionDetail, Return one bounded newest-first session page., Return bounded schema-aware detail for one session., Runtime surface needed by the current service API., Configure a control channel GPIO mode., Pulse the configured DUT reset role., Set the configured DUT boot/control role. (+11 more)
...
### Community 30 - "PatternDetector"
Cohesion: 0.10
Nodes (26): PatternDetector, PatternMatch, Pattern detection for completed UART log lines., A configured pattern found in one UART log line., Find configured text patterns in completed UART log lines., Configured patterns in scan order., Return all configured patterns present in one completed UART line., Return pattern matches for a sequence of completed UART lines. (+18 more)

### Community 31 - "test_enhanced.py"
Cohesion: 0.07
Nodes (37): AsyncEnhancedDeviceControl, AsyncEnhancedUartSender, EnhancedNdjsonEventStream, ControlState, Translate complete UART payloads through an async Enhanced transport., Parse Enhanced NDJSON chunks and expose only normalized evidence events., Return an incomplete Enhanced NDJSON frame buffered by the parser., Translate semantic control operations through an async Enhanced transport. (+29 more)
...
Cohesion: 0.14
Nodes (47): BufferStatusEvent, Enhanced-backend UART receive-buffer telemetry., Persist one buffer-status evidence unit., enhanced_snapshot(), evidence_bytes(), fixed_clock(), fixed_id(), datetime (+39 more)
...
### Community 38 - "workflows/capture.py"
Cohesion: 0.05
Nodes (27): CaptureReconnect, CaptureSessionStorage, CaptureSourceMonitor, CaptureWorkflowLifecycle, BaseException, Protocol, TracebackType, Backend-independent capture workflow coordination. (+19 more)
...
Cohesion: 0.13
Nodes (37): InvalidUtf8Error, MalformedMessageError, ProtocolError, ProtocolValidationError, ProtocolVersionError, ValueError, Typed failures at Enhanced wire-protocol boundaries., Base class for host-device protocol errors. (+29 more)

### Community 42 - "SessionPersistenceError"
Cohesion: 0.12
Nodes (29): BaselineOperation, clear_baseline(), _ineligibility_reason(), mark_baseline(), _persistence_fault(), datetime, Path, SessionDetail (+21 more)
...
Cohesion: 0.07
Nodes (38): _require_matching_enhanced_identity(), _require_matching_enhanced_snapshot(), BackendInputKind, BackendInfo, BackendInputError, BackendMode, Raised when a backend emits malformed or otherwise invalid input., Return the same classified failure with workflow-owned context. (+30 more)
...
Cohesion: 0.04
Nodes (52): backend_snapshot(), _project_event_health(), Service-owned continuous draining for finite capture workflows., Device Core Service startup configuration helpers., BlockingCaptureSource, ClosableFakeRuntime, _hardware_config(), NoopDeviceControl (+44 more)
...
Cohesion: 0.04
Nodes (56): Reference to a created debug session., SessionHandle, Path, SessionDetail, Create filesystem-backed debug sessions., Persist derived records finalized at segment or session close., Append one accepted normalized control action atomically., Durably append a forced-send attempt and reserve its result record. (+48 more)

### Community 58 - "test_report_boundary.py"
Cohesion: 0.28
Nodes (23): prepare_analysis(), ProviderSelection, Freeze the exact request and expose a manifest before any provider call., FixtureAdapter, asyncio, parametrize, Path, _request() (+15 more)
...
Cohesion: 0.10
Nodes (44): _validate_session_command(), EvidenceTypeCount, FirstErrorReference, LegacySessionListItem, Compact first-error location used by bounded session list items., Bounded read-only identity projection for one unversioned session., Logical size and optional record count for one session artifact., Bounded count for one detected-pattern or hardware-event type. (+36 more)

### Community 62 - "test_startup_config.py"
Cohesion: 0.19
Nodes (31): build_startup_runtime(), Path, Build the service runtime for one explicitly selected backend., backend_settings(), _enhanced_info(), _enhanced_segment(), FakeEnhancedAsyncHost, MonkeyPatch (+23 more)
...
Cohesion: 0.07
Nodes (42): CommandSuccessMessage, Successful command response from the Debug Helper., GpioConfigurator, GpioModeRequestSource, Configure GPIO modes through firmware and update accepted host state., Send `configure_gpio_mode` and record the firmware result., GpioConfigurationError, RuntimeError (+34 more)
...
### Community 71 - "CaptureRecorder"
Cohesion: 0.11
Nodes (28): CaptureRecorder, _normalized_control_timestamp(), Route normalized backend events into UART processing and session storage., Create a capture session and return a recorder for it., Handle for the session this recorder writes to., Session identifier this recorder writes to., Whether storage has already ended this recorder's native session., Observe terminalization performed by a coordinated external evidence writer. (+20 more)
...
Cohesion: 0.13
Nodes (37): CaptureWorkflow, Own the complete lifecycle of one finite capture session., EnhancedCaptureFixtureRecorder, FakeCaptureEventSource, FakeMonotonicClock, fixed_clock(), fixed_id(), LifecycleCaptureEventSource (+29 more)
...
### Community 83 - "uart_rx_adapter_harness.c"
Cohesion: 0.10
Nodes (26): dutchmate_cdc_tx_initialize(), command_uart_cancel(), dutchmate_command_runtime_end_epoch(), dutchmate_command_runtime_start_epoch(), dutchmate_platform_uart_interface_set(), dutchmate_uart_rx_faulted(), dutchmate_uart_rx_initialize(), dutchmate_uart_rx_start() (+18 more)
...
Cohesion: 0.14
Nodes (22): _context(), main(), _parser(), ArgumentParser, Path, Host-facing preview and digest-approved Debug Agent analysis., EvidenceLimits, AnalysisManifest (+14 more)

### Community 99 - "BackendSnapshot"
Cohesion: 0.12
Nodes (9): Accept ownership of one validated concrete replacement., BackendSnapshot, Backend identity, effective policy, timing, and integrity for one segment., CommandedBootMode, Exception, SessionWorkflow, Create one capture session., Append a validated reconnect segment and return its segment ID. (+1 more)
...
Cohesion: 0.19
Nodes (21): build_enhanced_capture_reconnect(), Keep the runtime UART-send port stable across backend replacement., Publish a newly connected UART-send adapter., Build coordinated Enhanced reopen through the async host boundary., ReplaceableUartSender, _enhanced_info(), _enhanced_settings(), FakeControl (+13 more)
...
Cohesion: 0.09
Nodes (17): BasicSerialFactory, BasicBackendEventSource, BackendEvent, _pyserial_factory(), Basic generic USB-to-UART connection, receive, and send adapter., Normalize raw Basic serial chunks through one FIFO reader thread., Return immutable Basic backend identity and capabilities., Return the session-local segment ID assigned to this source. (+9 more)
...
Cohesion: 0.19
Nodes (17): CliConfigError, DaemonConfig, _optional_port(), _optional_positive_int(), _optional_str(), _optional_table(), _parse_daemon_config(), _parse_sessions_config() (+9 more)
...
Cohesion: 0.23
Nodes (15): BackendConfig, Apply explicit overrides and backend-specific defaults., Configured UART values before backend-specific defaults are resolved., Optional project backend selection loaded from TOML., resolve_backend_settings(), UartConfig, parametrize, test_backend_mode_is_required_without_cli_or_config_value() (+7 more)

### Community 130 - "time_us_64"
Cohesion: 0.19
Nodes (19): cdc_callback(), dutchmate_cdc_tx_cancel(), dutchmate_cdc_tx_poll(), dutchmate_cdc_tx_start(), command_uart_start(), dutchmate_command_runtime_on_cdc_rx_ready(), dut_uart_callback(), mark_driver_fault() (+11 more)
...
Cohesion: 0.16
Nodes (28): BuildContext, ChangedFile, CodingContext, _commit(), ContextExcerpt, _diff_hunks(), DiffHunk, _excerpt() (+20 more)
...
Cohesion: 0.22
Nodes (26): build_analysis_request(), Versioned, bounded request assembled without provider calls., Combine native evidence with optional validated caller context., build_session_evidence(), Assemble bounded, priority-selected evidence without changing stored facts., _append_lines(), _native_session(), MonkeyPatch (+18 more)
...
### Community 151 - "CaptureRecordResult"
Cohesion: 0.20
...
### Community 153 - "test_session_facts.py"
Cohesion: 0.49
Nodes (9): load_session_facts(), Load facts through the validated, schema-aware session-store query., Path, _snapshot(), _store(), test_loads_bounded_native_facts_without_raw_artifacts(), test_rejects_legacy_session_without_inferred_native_facts(), test_rejects_path_traversal_session_id() (+1 more)

### Community 154 - "UartSendWorkflow"
Cohesion: 0.11
Nodes (16): BackendWriteError, Raised when a backend cannot accept a complete UART payload., ActiveUartSendSession, _format_utc(), BackendMode, datetime, Protocol, Send one validated payload, optionally as a recorded perturbation. (+8 more)
...
Cohesion: 0.07
Nodes (20): Apply startup hardware control mappings., Return the current Device Core status., Service entrypoint for DUTchMate., load_startup_hardware_config(), GpioRoleName, Protocol, Runtime surface needed to apply startup hardware configuration., Return current runtime status. (+12 more)
...
### Community 185 - "dutchmate_usb_connection_run"
Cohesion: 0.10
Nodes (19): dutchmate_command_runtime_discard_input(), dutchmate_command_runtime_faulted(), dutchmate_command_runtime_initialize(), dmh_connection_epoch_init(), dmh_connection_epoch_update(), main(), dutchmate_platform_control_port(), dutchmate_platform_io_force_safe() (+11 more)
...
Cohesion: 0.07
Nodes (24): _build_async_enhanced_capture_reconnect(), _CandidateCleanupFailure, _close_unpublished_candidate(), OpenCaptureReplacement, OpenEnhancedHost, BaseException, ControlState, Event (+16 more)
...
### Community 202 - "SegmentTimestamp"
Cohesion: 0.23
Nodes (18): Immutable timestamp provenance for one continuous connection segment., SegmentTimestamp, BackendEvent, Exception, FakeMonotonicClock, parametrize, Path, Catch capability admission before newer connected monitor health is adopted. (+10 more)
...
Cohesion: 0.36
Nodes (7): MonkeyPatch, test_dut_boot_mode_command_reports_service_error(), test_dut_boot_mode_command_sets_boot_mode(), test_dut_reset_command_pulses_reset_role(), test_dut_reset_command_reports_service_error(), test_format_boot_mode_result_renders_accepted_action(), test_format_reset_result_renders_accepted_action()
...
Cohesion: 0.09
Nodes (24): build_basic_capture_reconnect(), OpenBasicConnection, Build coordinated idle and active reopen for one selected Basic backend., Open one raw Basic connection from resolved settings., Return an opened Basic connection., _basic_settings(), FakeBasicSerial, test_basic_active_candidate_uses_coordinator_and_replaces_sender() (+16 more)
...
### Community 231 - "test_contracts.py"
Cohesion: 0.10
...
Cohesion: 0.08
Nodes (49): Raw UART bytes observed by a backend within one connection segment., UartReceiveEvent, Backend-independent UART receive processing to log lines and matches., Finalize and discard trailing derived state for one connection segment., Completed UART lines and pattern matches produced from one processing step., Convert captured UART byte messages into complete lines and pattern matches., Process one normalized UART receive event., Return an independent candidate state for atomic evidence admission. (+41 more)
...
- **What is the exact relationship between `Phase 1B Enhanced Backend` and `Normalized Backend Contract`?**
  _Edge tagged AMBIGUOUS (relation: implements) - confidence is low._
- **Why does `SessionStore` connect `SessionStore` to `DeviceCoreRuntime`, `test_log_replay.py`, `enhanced.py`, `store.py`, `test_evidence_package.py`, `SegmentContext`, `test_recovery.py`, `test_session_facts.py`, `DeviceCoreStatus`, `test_basic.py`, `fixed_clock`, `CaptureSourceHealth`, `SessionPersistenceError`, `runtime.py`, `load_baseline_context`, `test_device_core_uart_send.py`, `test_startup_config.py`, `persistence.py`, `test_capture_reconnect.py`, `CaptureRecorder`, `test_baseline.py`, `SegmentTimestamp`, `test_retention.py`, `CaptureWorkflow`, `test_reconnect_evidence.py`, `helpers.py`, `evidence_package.py`, `test_device_core_lifecycle.py`, `test_connection_monitoring.py`, `test_comparison.py`, `provider_boundary.py`, `BackendSnapshot`, `test_real_coordinator_discards_idle_basic_event_before_capture`, `FakeDeviceControl`, `UartReceiveEvent`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `SegmentContext` connect `SegmentContext` to `test_backend_reconnect.py`, `.__init__`, `ContinuousIngestionCoordinator`, `DeviceCoreRuntime`, `enhanced.py`, `metadata.py`, `store.py`, `test_evidence_package.py`, `DeviceCoreRuntimeError`, `test_recovery.py`, `test_session_facts.py`, `UartSendWorkflow`, `DeviceCoreStatus`, `fixed_clock`, `workflows/capture.py`, `EnhancedAsyncHost`, `CaptureSourceHealth`, `BackendInfo`, `runtime.py`, `SessionStore`, `test_startup_config.py`, `test_capture_reconnect.py`, `CaptureRecorder`, `SegmentTimestamp`, `CaptureWorkflow`, `test_reconnect_evidence.py`, `helpers.py`, `test_device_core_lifecycle.py`, `test_connection_monitoring.py`, `test_comparison.py`, `test_real_coordinator_discards_idle_basic_event_before_capture`, `ReplaceableUartSender`, `test_contracts.py`, `BasicBackendEventSource`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `UartReceiveEvent` connect `UartReceiveEvent` to `ContinuousIngestionCoordinator`, `test_enhanced_serial.py`, `DeviceCoreRuntime`, `test_log_replay.py`, `enhanced.py`, `store.py`, `test_evidence_package.py`, `SegmentContext`, `CaptureRecordResult`, `test_session_facts.py`, `test_enhanced.py`, `evidence.py`, `test_basic.py`, `fixed_clock`, `workflows/capture.py`, `EnhancedAsyncHost`, `runtime.py`, `test_device_core_uart_send.py`, `SessionStore`, `test_startup_config.py`, `persistence.py`, `test_capture_reconnect.py`, `CaptureRecorder`, `SegmentTimestamp`, `CaptureWorkflow`, `helpers.py`, `test_comparison.py`, `test_real_coordinator_discards_idle_basic_event_before_capture`, `ReplaceableUartSender`, `test_contracts.py`, `BasicBackendEventSource`?**
